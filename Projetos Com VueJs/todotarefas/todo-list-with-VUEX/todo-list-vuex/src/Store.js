...
export default new Vuex.Store({
  ...
  actions: {
    addTodo(context, payload) {
      context;
      let ultimoId =
        context.state.todos.lenght === 0
          ? 0
          : context.state.todos[context.state.todos.length - 1].id;
      console.log(payload);
    ...
    finishTodo(context, payload) {
      let obj = context.state.todos.find((item) => item.id === payload.id);
      obj.isCompleted = !obj.isCompleted;
      context.commit("FINISH_TODO", {
        idx: payload,
        obj: obj,
      });
    },
